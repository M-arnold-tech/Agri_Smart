    ...
    <div className="flex h-[calc(100vh-64px-3rem)] bg-surface rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in">
      <div className="hidden md:flex w-80 border-r border-gray-200 flex-col bg-background">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex items-center gap-4 p-4 pl-6 cursor-pointer hover:bg-surface-hover transition-colors border-b border-gray-200">
            <UserCircle2 size={40} className="text-text-muted" />
            <div className="flex-1 min-w-0">
              <h4 className="text-xs font-semibold text-text-main mb-1">
                Musanze Coop Group
              </h4>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 p-6 overflow-y-auto flex flex-col gap-4 bg-background/50">
          {messages.map((msg) => (
            <div
              key={msg.id}
              className={`flex w-full ${msg.sender === "farmer" ? "justify-end" : "justify-start"}`}
            >
              <div
              ...
              >
                ...
                <span
                  className={`block text-[10px] mt-2 text-right ${msg.sender === "farmer" ? "text-white/80" : "text-text-muted"}`}
                >
                  {msg.time}
                </span>
              </div>
