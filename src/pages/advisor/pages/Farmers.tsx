  ...
  if (isLoading && farmers.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center min-h-[60vh]">
        ...
        <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Loading your assignments...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-8 animate-fade-in font-sans">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-text-main mb-2 tracking-tight">My Assignments</h1>
          <p className="text-text-muted font-medium">Manage and support your assigned farmers across districts.</p>
        </div>
        <div className="relative w-full md:w-80 group">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3.5 bg-white border border-gray-100 rounded-2xl text-sm focus:ring-4 focus:ring-primary/10 shadow-sm outline-none transition-all placeholder:text-gray-400 font-medium"
            value={searchTerm}
      ...
      {filteredFarmers.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredFarmers.map((farmer) => (
            <Card key={farmer.id} className="group p-0 border-none shadow-sm bg-white overflow-hidden rounded-[32px] hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 hover:-translate-y-1 border border-transparent hover:border-gray-100">
              <div className="p-6 pb-4">
                ...
                <div className="flex items-center gap-1.5 text-text-muted mb-4">
                  ...
                  <span className="text-xs font-bold uppercase tracking-wide">{farmer.district} District</span>
                </div>
        ...
        <div className="flex flex-col items-center justify-center p-20 text-center bg-white rounded-[40px] border-2 border-dashed border-gray-100">
           ...
           <h3 className="text-xl font-bold text-text-main mb-2">No Farmers Found</h3>
           <p className="text-text-muted max-w-xs font-medium">Try adjusting your search or contact the admin for new assignments.</p>
        </div>
      )}
    </div>
  );
