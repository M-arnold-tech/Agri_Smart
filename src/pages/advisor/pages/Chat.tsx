    ...
    <div className="flex h-[calc(100vh-140px)] bg-white rounded-[32px] overflow-hidden shadow-sm border border-gray-100 animate-fade-in font-sans">
      ...
      <div className="w-full md:w-[380px] border-r border-gray-50 flex flex-col bg-white shrink-0">
        <div className="p-6 pb-2">
           <h2 className="text-xl font-black text-text-main tracking-tight mb-6">Messages</h2>
           <div className="relative group mb-4">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-primary transition-colors" size={18} />
              <input 
                 type="text" 
                 placeholder="Search farmers..." 
                 className="w-full pl-12 pr-4 py-3.5 bg-gray-50/80 border-none rounded-2xl text-sm focus:ring-2 focus:ring-primary/20 outline-none transition-all placeholder:text-gray-400 font-medium"
                 value={searchTerm}
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-6">
           {isLoading && conversations.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-12 text-center opacity-40">
                 ...
                 <p className="text-[10px] font-bold uppercase tracking-widest">Gathering chats...</p>
              </div>
           ) : (
              conversations
                 ...
                 >
                    ...
                    <div className="flex-1 min-w-0">
                       <div className="flex items-center justify-between mb-1">
                          <h4 className={`text-sm font-bold truncate tracking-tight transition-colors ${activeChat?.id === chat.id ? "text-primary" : "text-text-main"}`}>
                             {chat.name}
                          </h4>
                       ...
                       <p className={`text-xs truncate font-medium ${activeChat?.id === chat.id ? "text-primary/60" : "text-gray-400"}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white">
        {activeChat ? (
          <>
            <header className="px-6 py-4 border-b border-gray-50 flex items-center justify-between z-10 bg-white/80 backdrop-blur-md">
              <div className="flex items-center gap-4">
                <Button variant="ghost" className="md:hidden p-2 rounded-full hover:bg-gray-100">
                  <ArrowLeft size={20} />
                </Button>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-black text-text-main tracking-tight leading-none mb-1">
                      {activeChat.name}
                    </h3>
                    ...
                    >
                      <div className={`flex flex-col max-w-[75%] ${isMe ? "items-end" : "items-start"}`}>
                        ...
                        <div className={`flex items-center gap-1.5 mt-2 px-1 ${isMe ? "justify-end" : "justify-start"}`}>
                          ...
                          {isMe && (
                             msg.isOptimistic ? <Clock size={10} className="text-gray-300" /> : <Check size={10} className="text-primary" />
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 text-center bg-gray-50/20">
            <div className="w-24 h-24 rounded-[40px] bg-primary/5 flex items-center justify-center text-primary mb-8 animate-bounce shadow-xl shadow-primary/5">
              <SendHorizonal size={40} />
            </div>
            <h3 className="text-xl font-black text-text-main mb-3 uppercase tracking-widest">Your Messaging Hub</h3>
            <p className="text-text-muted max-w-[280px] leading-relaxed text-sm font-medium opacity-70">
              Select a farmer from the inquiry list to provide guidance or answer questions.
            </p>
          </div>
        )}
      </div>
